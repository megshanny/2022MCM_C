...
def predict(data_price):
    ...
    # data_need_predict_s = pd.Series(data_need_predict).dropna()
    
    # ADF检验, 平稳性检验，大于0.05不稳定
    # print('ADF检验结果：',ADF(data1[rowname1])[1])
    # if ADF(data_need_predict)[1] > 0.05:
    #     data_need_predict = np.diff(data_need_predict)
    #     diff_count+=1

    model = ARIMA(data_need_predict,order=(1, 0, 1))
    model_fit = model.fit()
    data_predict = model_fit.predict(len(data_need_predict), len(data_need_predict)+pre_length-1)

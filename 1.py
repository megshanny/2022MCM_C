...
all_model.add(LSTM(4,batch_input_shape=(1,1,1),stateful=True)) #神经元个数为4
all_model.add(Dense(1))
all_model.compile(loss='mean_squared_error',optimizer='adam')
...
def predict(data_price):
    ...
    for i in range(5): #训练5次
        model.fit(X, y, batch_size=1, verbose=1, shuffle=False)
        model.reset_states()
    for i in range(0,pre_length):
        data_predict = model.predict(data_diff[-1:].reshape(-1,1,1),batch_size=1)
        data_diff = np.append(data_diff, data_predict)
...
if len(sys.argv) >= 1:
    ...
    while(i < len(sys.argv)):
        if(sys.argv[i] == '--day'):
            ...
            i += 2
        elif(sys.argv[i] == '--data_length'):
            data_length = int(sys.argv[i+1])
            print('data_length set to', data_length)
            i += 2

...
data = pd.read_csv('D:\Files\code\Python\MCM\workspace\BCHAIN-MKPRU.csv')
rowname = 'Value'
...
for i in range(pre_start, data['RowNumber'].max()+2):
    ...
    if(data_start < 0):
        data_start = 0
    data_slice = data[data_start:data_end][rowname].to_numpy()
    p = GM_11_fit(data_slice)
    MSER_SUM += GM_11_posttest(data_slice, p)[0]
    pre_data_temp = []
    pre_data_day_temp = []
    for j in range(0, pre_duration):
        if(data_end + 1 + j > data['RowNumber'].max()):
            break
        pre_data_temp.append(GM_11_predicate(data_slice[0], p, data_slice.__len__() + j))
    ...
    if pre_data_temp.__len__() != 0:
        ...
        data_true = data[(data['RowNumber'] >= pre_data_day_temp[0]) & (data['RowNumber'] <= pre_data_day_temp[-1])][rowname].to_numpy()
...
plt.plot(data['Date'], data[rowname], color='black')
...
def update(val):
    ...
    data_true = data[(data['RowNumber'] >= pre_data_day[pos][0]) & (data['RowNumber'] <= pre_data_day[pos][-1])][rowname].to_numpy()
